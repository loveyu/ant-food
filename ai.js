var AI = {
	...
	get_search_food: function (ant) {
		...
		var nx = 0, ny = 0, mx = 0, i, j, x2, flag = false, jx, jy;
		for (i = 0 - ant.eye; i <= ant.eye; i++) {
			for (j = 0 - ant.eye; j <= ant.eye; j++) {
				...
				var xob = {x: nx, y: ny};
				if (AI.is_in_food(xob) && !AI.is_out_wall(ant, xob) && !AI.is_in_water(xob)) {
					mx = -1;
					flag = true;
					break;
				}
				if (AntFood.food_map.hasOwnProperty(x2)) {
					if (AntFood.food_map[x2] > mx && !AI.in_queue(ant.queues, xob) && !AI.is_out_wall(ant, xob) &&
						!AI.is_in_water(xob)) {
	...
	search_water: function (ant) {
		ant = AI.save_home_info(ant);
		var nx = 0, ny = 0, i, j, x2, flag = false, xob, mx = 0, jx, jy;
		for (i = 0 - ant.eye; i <= ant.eye; i++) {
			for (j = 0 - ant.eye; j <= ant.eye; j++) {
				if (i == 0 && j == 0)continue;
				nx = ant.x + i;
				...
				xob = {x: nx, y: ny};
				x2 = ny * AntFood.width + nx;
				if (AI.is_in_water(xob) && !AI.is_out_wall(ant, xob) && !AI.is_in_food(xob)) {
					flag = true;
					break;
				}
				if (AntFood.water_map.hasOwnProperty(x2)) {
					if (AntFood.water_map[x2] > mx && !AI.in_queue(ant.queues, xob) && !AI.is_out_wall(ant, xob) &&
						!AI.is_in_food(xob) && !AI.is_in_home(xob)) {
	...
	get_back_rule: function (ant) {
		...
		for (i = 0 - ant.eye; i <= ant.eye; i++) {
			for (j = 0 - ant.eye; j <= ant.eye; j++) {
				...
				x2 = ny * AntFood.width + nx;
				var xob = {x: nx, y: ny};
				if (AI.is_in_home(xob) && !AI.is_out_wall(ant, xob) && !AI.is_in_food(xob) && !AI.is_in_water(xob)) {
					mx = -1;
					flag = true;
					break;
				}
				if (AntFood.home_map.hasOwnProperty(x2)) {
					if (AntFood.home_map[x2] > mx && !AI.in_queue(ant.queues, {x: nx, y: ny}) &&
						!AI.is_out_wall({x: ant.x, y: ant.y}, xob) && !AI.is_in_food(xob) && !AI.is_in_water(xob)) {
	...
	in_queue: function (queue, obj) {
		for (var i in queue) {
			if (queue.hasOwnProperty(i)) {
				if (queue[i].x == obj.x && queue[i].y == obj.y)
					return true;
			}
		}
		return false;
	...
	get_ant_search_rul: function (ant) {
		...
		var i = 0;
		while (AI.is_out_wall({x: ant.x, y: ant.y}, {x: next.x, y: next.y}) || AI.in_queue(ant.queues, next) ||
		AI.is_in_water(next) || AI.is_in_food(next)) {
			if (++i > 100) {
				ant.queues = [];
				return ant;
			}
	...
	is_out_wall: function (p1, p2) {
		for (var i in AntFood.wall) {
			...
			var status = AI.line_x_node(p1, p2, {x: obj.x1, y: obj.y1}, {x: obj.x2, y: obj.y2});
			if (status) {
				return true;
			}
		}
	...
	in_circle: function (circle, p) {
		return (circle.x - p.x) * (circle.x - p.x) + (circle.y - p.y) * (circle.y - p.y) <= circle.r * circle.r;
	},
	...
	line_x_node: function (p1, p2, q1, q2) {
		...
		var d2 = ((q2.x - q1.x) * (p1.y - q1.y) - (q2.y - q1.y) * (p1.x - q1.x)) *
	...
	save_home_info: function (ant) {
		...
		if (now < AntFood.c_home)
			AntFood.home_map[x] = now + ant.c_home * 0.001;
		ant.c_home *= 0.999;
		...
		if (now < AntFood.c_food)
			AntFood.food_map[x] = now + ant.c_food * 0.01;
		ant.c_food *= 0.99;
